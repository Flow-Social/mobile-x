...
class ChatInputController internal constructor(
	...
	val keyboardLayoutHeightPx: Int
		get() = when (uiState) {
			ChatInputUiState.KeyboardRequested -> when {
				imeHeightPx > 0 -> imeHeightPx
				shouldRestoreKeyboardOnResume -> max(stableKeyboardHeightPx, fallbackPanelHeightPx)
				else -> 0
			}
	...
	private fun String.trimToMaxLength(): String {
		return if (maxLength != null && codePointLength() > maxLength) {
			substring(0, offsetByCodePointsSafe(maxLength))
		} else {
			this
		}
	}
	...
	private fun logState(event: String) {
		// Common engine intentionally stays platform-neutral.
	}
}
...
private fun previousCodePointStart(text: String, index: Int): Int =
	text.offsetByCodePointsSafe(-1, index)
...
private fun String.codePointAt(index: Int): Int {
	...
	return if (
		...
		((first.code - 0xD800) shl 10) + (this[index + 1].code - 0xDC00) + 0x10000
	} else {
		first.code
	}
}

private fun String.offsetByCodePointsSafe(codePointOffset: Int, startIndex: Int = 0): Int {
	...
	repeat(-codePointOffset) {
		if (index <= 0) return 0
		index -= 1
		if (index > 0 && this[index - 1].isHighSurrogate() && this[index].isLowSurrogate()) {
			index -= 1
		}
	}
	return index
