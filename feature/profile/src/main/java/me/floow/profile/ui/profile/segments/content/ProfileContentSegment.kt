...
import me.floow.profile.ui.profile.components.ProfilePostCard
...
private val ProfilePostsGridContentPadding = PaddingValues(
	bottom = 24.dp,
	start = 12.dp,
	end = 12.dp,
	top = 12.dp,
...
fun ProfileContentSegment(
	...
	onPostLongClick: (Post, PostMediaSourceSnapshot?) -> Unit = { _, _ -> },
	onPostClick: (Post, PostMediaSourceSnapshot?) -> Unit = { _, _ -> },
	...
	when {
		arePostsLoading && posts.isEmpty() -> {
			ProfilePostsSkeletonGrid(
				alpha = skeletonAlpha,
				modifier = modifier
			)
		}
		...
		posts.isEmpty() -> {
			...
			LazyVerticalGrid(
				...
				if (isLoadingMorePosts) {
					item(
						key = "profile_posts_loading_more",
...
@Composable
private fun ProfilePostsSkeletonGrid(
	alpha: Float,
	modifier: Modifier = Modifier
) {
	LazyVerticalGrid(
		...
		items(count = ProfileSkeletonItemsCount, key = { index -> "profile_posts_skeleton_$index" }) {
			ProfilePostSkeletonCard(alpha = alpha)
		}
	}
}
...
@Composable
private fun ProfilePostSkeletonCard(alpha: Float) {
	Box(
		modifier = Modifier
			.fillMaxWidth()
